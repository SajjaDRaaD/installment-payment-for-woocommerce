...
function Load_ZarinPal_Installment_Gateway()
{

    if (!function_exists('Woocommerce_Add_ZarinPal_Installment_Gateway') && class_exists('WC_Payment_Gateway') && !class_exists('WC_ZPal_Inst')) {


        add_filter('woocommerce_payment_gateways', 'Woocommerce_Add_ZarinPal_Installment_Gateway');

        function Woocommerce_Add_ZarinPal_Installment_Gateway($methods)
        {
            $methods[] = 'WC_ZPal_Inst';
            return $methods;
        }
        ...
        class WC_ZPal_Inst extends WC_Payment_Gateway
        {
            ...
            public function __construct()
            {
                ...
                $this->id = 'WC_ZPal_Inst';
                $this->method_title = __('پرداخت اقساطی', 'woocommerce');
                $this->method_description = __('تنظیمات پرداخت اقساطی', 'woocommerce');
                $this->icon = apply_filters('WC_ZPal_Inst_logo', WP_PLUGIN_URL . '/woocommerce-cheque/assets/images/logo.png');
            ...
            {
                $this->form_fields = apply_filters('WC_ZPal_Inst_Config', array(
                        'base_config' => array(
            ...
            public function Send_to_ZarinPal_Gateway($order_id)
            {
                ...
                $currency = apply_filters('WC_ZPal_Inst_Currency', $currency, $order_id);
                $orderStatus = $order->get_status();
                ...
                $form = apply_filters('WC_ZPal_Inst_Form', $form, $order_id, $woocommerce);

                do_action('WC_ZPal_Inst_Gateway_Before_Form', $order_id, $woocommerce);
                echo $form;
                do_action('WC_ZPal_Inst_Gateway_After_Form', $order_id, $woocommerce);


                $pish_pardakht = get_post_meta( $order_id, 'pish_pardakht', true );
                ...
                $CallbackUrl = add_query_arg('wc_order', $order_id, WC()->api_request_url('WC_ZPal_Inst'));
                ...
                $Description = apply_filters('WC_ZPal_Inst_Description', $Description, $order_id);
                $Mobile = apply_filters('WC_ZPal_Inst_Mobile', $Mobile, $order_id);
                $Email = apply_filters('WC_ZPal_Inst_Email', $Email, $order_id);
                $Payer = apply_filters('WC_ZPal_Inst_Paymenter', $Payer, $order_id);
                $ResNumber = apply_filters('WC_ZPal_Inst_ResNumber', $ResNumber, $order_id);
                do_action('WC_ZPal_Inst_Gateway_Payment', $order_id, $Description, $Mobile);
                ...
                if (!empty($Message) && $Message) {
                    ...
                    $Note = apply_filters('WC_ZPal_Inst_Send_to_Gateway_Failed_Note', $Note, $order_id, $Fault);
                    $order->add_order_note($Note);


                    $Notice = sprintf(__('در هنگام اتصال به بانک خطای زیر رخ داده است : <br/>%s', 'woocommerce'), $Message);
                    $Notice = apply_filters('WC_ZPal_Inst_Send_to_Gateway_Failed_Notice', $Notice, $order_id, $Fault);
                    if ($Notice) {
                        wc_add_notice($Notice, 'error');
                    }

                    do_action('WC_ZPal_Inst_Send_to_Gateway_Failed', $order_id, $Fault);
                }
            }
            ...
            public function Return_from_ZarinPal_Gateway()
            {
                ...
                if ($order_id) {
                    ...
                    if ($order->status !== 'completed') {
                        ...
                        if ($Status === 'completed' && isset($Transaction_ID) && $Transaction_ID !== 0) {
                            ...
                            $Note = apply_filters('WC_ZPal_Inst_Return_from_Gateway_Success_Note', $Note, $order_id, $Transaction_ID);
                            if ($Note)
                            ...
                            $Notice = apply_filters('WC_ZPal_Inst_Return_from_Gateway_Success_Notice', $Notice, $order_id, $Transaction_ID);
                            if ($Notice)
                                wc_add_notice($Notice, 'success');

                            do_action('WC_ZPal_Inst_Return_from_Gateway_Success', $order_id, $Transaction_ID);
                        ...
                        $Note = apply_filters('WC_ZPal_Inst_Return_from_Gateway_Failed_Note', $Note, $order_id, $Transaction_ID, $Fault);
                        if ($Note) {
                            $order->add_order_note($Note, 1);
                        }
                        ...
                        $Notice = apply_filters('WC_ZPal_Inst_Return_from_Gateway_Failed_Notice', $Notice, $order_id, $Transaction_ID, $Fault);
                        if ($Notice) {
                            wc_add_notice($Notice, 'error');
                        }

                        do_action('WC_ZPal_Inst_Return_from_Gateway_Failed', $order_id, $Transaction_ID, $Fault);
                    ...
                    $Notice = apply_filters('WC_ZPal_Inst_Return_from_Gateway_ReSuccess_Notice', $Notice, $order_id, $Transaction_ID);
                    if ($Notice) {
                        wc_add_notice($Notice, 'success');
                    }

                    do_action('WC_ZPal_Inst_Return_from_Gateway_ReSuccess', $order_id, $Transaction_ID);
                ...
                $Notice = apply_filters('WC_ZPal_Inst_Return_from_Gateway_No_Order_ID_Notice', $Notice, $order_id, $Fault);
                if ($Notice) {
                    wc_add_notice($Notice, 'error');
                }

                do_action('WC_ZPal_Inst_Return_from_Gateway_No_Order_ID', $order_id, '0', $Fault);
...
add_action('plugins_loaded', 'Load_ZarinPal_Installment_Gateway', 0);
